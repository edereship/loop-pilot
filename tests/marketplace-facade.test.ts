...
// fan out to the reusable workflows team-yubune/loop-pilot/.github/workflows/
...
describe("marketplace facade: stays an inert signpost (never a 2nd code path)", () => {
  ...
  it("references no LoopPilot sub-action or local ./ action, so it cannot run the loop", () => {
    ...
    expect(rootActionCode).not.toMatch(/uses:\s*["']?team-yubune\/loop-pilot/);
  });

  it("points users at the real install paths (gh looppilot CLI)", () => {
    expect(rootAction).toContain("gh extension install team-yubune/gh-looppilot");
  });
});
...
describe("marketplace facade: README documents the listing as a non-consumption front door (TY-343 AC #3/#4)", () => {
  ...
  it("both READMEs warn that the bare root ref is not the way to run LoopPilot", () => {
    ...
    // `uses: team-yubune/loop-pilot@v1` (bare root ref), which only prints guidance.
    // (The workflow refs are `team-yubune/loop-pilot/.github/...@v1`; the bare
    // `team-yubune/loop-pilot@v1` substring appears only in this warning.)
    expect(readmeEn).toContain("team-yubune/loop-pilot@v1");
    expect(readmeJa).toContain("team-yubune/loop-pilot@v1");
  });
});
