...
**設計への影響:**
- [Workflow B](../architecture/event-design.md#workflow-b-codex-レビュー受信--claude-修正auto-review-loopyml) の主トリガーは **`pull_request_review.submitted`** とする。実測で Codex の総評がこのイベントとして到達したため
...
パーサーは **P0 / P1 / P2 / P3** を識別する（TY-256）。実際に修正対象とする範囲は `AUTO_REVIEW_SEVERITY_THRESHOLD`（default `P2`）で制御する。

- `AUTO_REVIEW_SEVERITY_THRESHOLD=P2`（既定）: P0 / P1 / P2 を修正対象、P3 を skip（旧挙動互換）
- `AUTO_REVIEW_SEVERITY_THRESHOLD=P3`: P0..P3 すべてを対象
- `AUTO_REVIEW_SEVERITY_THRESHOLD=P1`: P0 / P1 のみ、P2 / P3 を skip
- `AUTO_REVIEW_SEVERITY_THRESHOLD=P0`: P0 のみ

不正値は warning ログを出して default (`P2`) にフォールバックする。

severity を skip した件数は observability ログで出力する（`unparseable` / `belowThreshold` に分離）。詳細は [event-design.md Phase 1](../architecture/event-design.md#workflow-b-codex-レビュー受信--claude-修正auto-review-loopyml) を参照。
...
- 実際に修正対象とする range は `AUTO_REVIEW_SEVERITY_THRESHOLD` で制御する（上記「対象 severity」参照）
...
- P0/P1 の auto-fix 対象例
- P2 の非対象例
- `P0` / `P1` という語を含むが非対象の総評・説明文
