...
- Claude は findings ごとに1回以上の `edit_file` ツール呼び出しを返す
...
1ファイルに複数の `edit_file` が返された場合、**ファイル末尾側の edit から逆順に適用する**。先頭側の edit を先に適用すると行数の増減により後続 edit の `old_code` マッチ位置がズレるためである。Codex が報告する `line` はレビュー時点の行番号であり、先行する edit による行数変動を反映しないことに注意

### edit 間の依存関係

Claude はファイル全文を見て全 edit を一括生成する。この時点では先行 edit は適用されていないため、Claude が生成する各 `old_code` は**元のファイル内容に基づいている**。逆順適用はこの前提と整合する設計である。ただし、edit A の `new_code` が edit B の `old_code` に影響するケース（例: edit A で関数名を変更し、edit B がその関数を参照する行を修正する場合）では不整合が起きうる。このケースは `old_code` 不一致としてフォールバック（再試行）で対処する

### 置換対象が見つからない場合の段階的フォールバック

1. **再試行（最大2回）:** エラーメッセージ（「`old_code` が一致しない」旨 + **先行 edit をメモリ上で適用済みの中間状態のファイル内容**）を Claude に返し、修正版の `edit_file` を再要求する。先行 edit 適用後の状態を送ることで、Claude が最新のファイル内容に基づいた正確な `old_code` を生成できる。リトライで返された `edit_file` は、先行 edit 適用済みのメモリ上のファイルに対して検証する。再試行を2回としたのは、大きなファイルでは1回の再試行で `old_code` を正確に出力できないことが多いためである（コスト増は軽微: 最大 ~$0.15 / ファイル）
2. **スキップ:** 再試行でも一致しない場合、そのファイルの edit をスキップし、PR コメントに「適用できなかった修正」として報告する
3. **ロールバック:** ファイルへの edit 適用は **全 edit 成功後に一括書き込み** する（途中適用を防ぐため、メモリ上で全 edit を検証してからディスクに反映する）

---

## 大量の findings・大きなファイルへの対応

- findings をファイル単位でグループ化し、**ファイルごとに API を呼び出す**
- 各呼び出しでは、対象ファイルの全文 + そのファイルに関する findings のみを送信する
- これにより、1回あたりのコンテキストを抑えつつ、ファイル内の文脈を十分に与えられる
- **クロスファイル修正の制約（PoC）:** ファイル単位の API 呼び出しでは、1つの finding が複数ファイルにまたがる修正を要求するケース（例: 関数シグネチャ変更 + 呼び出し側の修正）に対応できない。PoC 段階では **ファイル単位で閉じた修正のみ対応** し、クロスファイル修正が必要な finding は PR コメントに「手動対応が必要」として報告する。本番移植時には、依存関係のあるファイルを同一バッチで送る仕組みを検討する
- **ファイル数上限:** 1 iteration あたりの対象ファイル数が `MAX_FILES_PER_ITERATION`（デフォルト: 10）を超える場合、以下の優先順位で対象ファイルを選択する:
  1. P0 findings を含むファイル（P0 と P1 が混在するファイルもこのグループに含める）
  2. P1 のみのファイル
  3. 同一グループ内では findings 数が多いファイルを優先する
...
- リトライは **ファイル単位** で行う。ファイル A の API 呼び出しが失敗しても、ファイル B の処理は独立して続行する
- **全ファイル失敗** した場合: `status: stopped`, `stop_reason: claude_api_error` で停止
- **一部ファイル成功** した場合: 成功したファイルの edit のみ適用し、失敗したファイルは PR コメントに「修正できなかったファイル」として報告する。commit / push は成功分のみで行い、次の iteration で残りを再試行する機会を与える
...
`{findings_json}` には対象ファイルに関する findings のみを埋め込む:
