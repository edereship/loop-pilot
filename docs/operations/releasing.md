...
`uses: Edership/loop-pilot/{init,loop}@v1` の形で参照する。
...
- **ルート `action.yml`（TY-343）** — `Edership/loop-pilot@v1`（サブパスなし）の
...
- **サブアクション参照整合**: `npm run check:action-refs -- v1.2.3`。
  `loop/action.yml`（サブアクション `Edership/loop-pilot/loop/{pre-fix,post-fix}@v1`）と
  再利用ワークフロー `.github/workflows/{loop,init}.yml`（`Edership/loop-pilot/{loop,init}@v1`）の
  ...
  に含めない）。inert な看板で `Edership/loop-pilot/...@v<major>` 参照を一切持たないため、
...
外部リポジトリから `uses: Edership/loop-pilot/init@v1` が解決・実行できることを
