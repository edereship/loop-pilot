...
|---|---|---|
| **`CLAUDE_CODE_MODEL`** | unset (空文字) | **hard override**。設定されると tiering を完全に無効化し、毎 iteration このモデルを使う。TY-241 以前の挙動 (常に Opus) を再現したい場合は `claude-opus-4-7` を明示設定する |
...
- **override は維持**: 既存 ops が `CLAUDE_CODE_MODEL=claude-opus-4-7` を明示設定している運用を尊重し、tiering 切替時に静かな挙動変化を起こさない
...
`override` tier で repair した場合は記録上 `"escalated"` として保存する (override は固定モデルで escalation の余地が無いため、次回 hash 一致時に確実に停止させる)。
