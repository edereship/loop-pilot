...
**PoC / TY-145 実測:** fork guard は Workflow A/B に実装済み。PR #7 / #58 は同一リポジトリ PR のため、checkout、Claude 修正、`CHECK_COMMAND`、commit/push、再レビュー、`done / no_findings` まで確認済み。2026-05-16 に外部 fork PR E2E を試みたが、この private repo は GitHub API が `forking is disabled` を返すため、外部 fork PR の実機確認は本 repo では未完了。本番対象 repo で fork PR を作成し、secrets / checkout / auto-fix が動かないことを確認する。詳細手順は [Production E2E Validation Notes](production-e2e-validation.md#external-fork-pr-validation) を参照。
...
**Workflow への wiring:**
- Repository variable は workflow run の env に **自動マップされない**。本リポジトリの `looppilot-loop.yml` / `looppilot-init.yml` は composite action の input `looppilot-state-comment-authors` に `${{ vars.LOOPPILOT_STATE_COMMENT_AUTHORS }}` を渡し、`loop/action.yml` がそれを `init` / `loop/pre-fix` / `loop/post-fix` に forward する。Node 側は `core.getInput("looppilot-state-comment-authors")` を最優先で読み、未設定なら従来の `LOOPPILOT_STATE_COMMENT_AUTHORS` env にフォールバックする
...
本番リポジトリで意図しない PR に Codex review / Claude auto-fix loop が走らないよう、デフォルトで「`loop-pilot` ラベルが付いた PR でのみ起動」する仕様（TY-137）。完全自動化したい場合のみ opt-out できる。
...
**必要な権限:**
- `contents: write`（commit / push、および `LOOPPILOT_AUTO_MERGE=true` の場合は `gh pr merge <pr> --squash --match-head-commit <sha>` 呼び出し。auto-merge の CI gating は branch protection に依存せず自前で実装、TY-277）
- `pull-requests: write`（コメント投稿、および auto-merge オプション有効時のマージ実行）
- `issues: write`（hidden comment の読み書き）
- `actions: read`（`LOOPPILOT_AUTO_MERGE=true` 時のみ必須。`mergeIfChecksPass` が `/repos/.../actions/runs?head_sha=...` を polling して CI 結果を確認するため。未付与だと API が 403 を返し auto-merge が常に skip される、TY-277）
...
`anthropics/claude-code-action@v1` の認証は **2 方式** から選択する (TY-260)。両方とも GitHub Actions の **Repository secrets** に保存し、wrapping workflow が `loop` composite action に渡す。

| Secret | 用途 | 課金 |
|---|---|---|
| `ANTHROPIC_API_KEY` | Anthropic API 直接呼び出し | Anthropic API 従量課金 |
| `CLAUDE_CODE_OAUTH_TOKEN` | Claude Code サブスク (Pro / Max) 経由 | サブスク使用量上限を消費 |
...
- **TY-272 #B**: `LOOPPILOT_PUSH_TOKEN` を使った push 経路では、`pushWithToken` が `http.extraheader=AUTHORIZATION: Basic <base64>` の **base64 形式も `core.setSecret` で登録** する。素のトークンは `registerAllSecrets` でマスク済みだが、`Buffer.from("x-access-token:" + token).toString("base64")` は派生 secret として別文字列扱いになるため、git の失敗時 stderr が argv をエコーした場合のログ漏洩経路を防ぐ
- Fork PR の workflow ではシークレットにアクセスできない (GitHub のデフォルト挙動で保護される)
- 認証情報のローテーション手順は API キー / OAuth トークン共に本番移植時に定める

### サブスク利用時の注意

サブスク (OAuth トークン) は **個人アカウントの使用量上限** に従う。CI 用途では以下に注意:

- **使用量上限が早く枯渇する**: LoopPilot loop は 1 PR で最大 20 iteration まで Claude を呼ぶため、サブスクの時間枠 quota (Pro / Max いずれも) を一気に消費する可能性がある。特に Opus escalation 経路は消費が早い
- **個人のインタラクティブ使用と quota を共有**: 同じアカウントで Claude Code を手元で使っていると、CI と quota を取り合うことになる
...
Actions の required check が発火しない場合がある。TY-145 の disposable
public repo E2E でこの挙動を確認したため、Repository secret
`LOOPPILOT_PUSH_TOKEN` を任意で設定できる。

**TY-281 検証結果 (2026-05-17)**: PR #88 で `LOOPPILOT_PUSH_TOKEN` を
...
のため、CI が走った事実が `LOOPPILOT_PUSH_TOKEN` 経路で push されている
ことの決定的証拠となる。
...
TY-145 の 2026-05-16 検証では、この repository は private non-Pro 制約により branch protection / rulesets API が 403 を返した。

```text
HTTP 403: Upgrade to GitHub Pro or make this repository public to enable this feature.
```

そのため、本 repo では branch protection 下での auto-fix push 可否を実証できない。production repository では以下を確認する:
...
具体的な確認手順は [Production E2E Validation Notes](production-e2e-validation.md#branch-protection-and-rulesets) を参照。
...
### モデル選定 (TY-241: Sonnet default + Opus escalation tiering)
...
|---|---|---|
| **`CLAUDE_CODE_MODEL_BASE`** | `claude-sonnet-4-6` | base tier。escalation 条件が立たない iteration に使う |
| **`CLAUDE_CODE_MODEL_ESCALATED`** | `claude-opus-4-7` | escalated tier。下記 escalation 条件のいずれかが立つと使う |

「常に同じモデルを使う」運用 (例: 常時 Opus、常時 Sonnet) は `BASE` と `ESCALATED` に同じ値を設定することで表現する。tiering 自体を無効化する専用 override は **TY-242 で廃止** した (旧 `CLAUDE_CODE_MODEL` 変数は無くなった)。
...
- **base = Sonnet**: TY-241 までの PoC (PR #34 / #35) で観測した修正タスクの大半は Sonnet 4.6 で十分到達可能なクラス。常時 Opus はコスト過剰
...
#### `findingsHashHistory` の tier 記録 (TY-243)
...
- `modelTier` 未設定の legacy entry は `"escalated"` として扱う (TY-243 以前の state との後方互換: legacy 動作 = 即停止 を維持)
...
- **root dotfile の hard-block は root レベルのみ** (TY-276 #10): パターン `/^\.[^/]+$/` で `^\.` を要求するため、`.env` / `.gitignore` は弾かれるが **`tests/.env` / `src/.env` / `docs/.npmrc` のようなネスト dotfile は対象外** で block されない。`tests/` 配下に `.env` を fixture として置く運用がある場合は、`LOOPPILOT_BLOCK_PATHS=tests/.env` のように明示的に block を追加するか、`scope-additional-hard-block-prefixes` で path を hard-block 化する
- 解除運用は audit log として残るよう Repository variable で管理し、PR ラベル等での動的 override は導入しない

### Secrets / token / workflow permissions

- claude-code-action に渡す認証情報は **`ANTHROPIC_API_KEY` または `CLAUDE_CODE_OAUTH_TOKEN` の片方のみ** (両方セット時は pre-fix が fail fast。TY-260)
...
TY-241 で base = Sonnet / escalated = Opus の階層化を導入したため、P0 / check_failed が立たない iteration は Sonnet で実行され実コストは上表の Sonnet 行ベースに近づく見込み。`loop-pilot` label 付き PR が月 5 件で大半が base tier に収まる想定なら **\$20〜100/月** 程度。これを超える運用規模になれば `CLAUDE_CODE_MODEL_BASE=claude-haiku-4-5-20251001` に下げる、`MAX_REVIEW_ITERATIONS` を `10` に絞る等で調整可能。
