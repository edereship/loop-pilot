...
LoopPilot は 4 つの認証情報を扱いますが、**`GITHUB_TOKEN` は GitHub Actions が自動で供給するため、あなたが作成・保存することはありません。** 実際に用意するものは少しだけです。

- **あなたが用意するもの**（GitHub Actions の **Repository secrets** に保存）: Anthropic 認証を 1 つ — `ANTHROPIC_API_KEY` か `CLAUDE_CODE_OAUTH_TOKEN`（**必須**）— に加えて、本番向けに任意の PAT `CODEX_REVIEW_REQUEST_TOKEN` と `LOOPPILOT_PUSH_TOKEN`。
- **自動・設定不要:** `GITHUB_TOKEN`。Actions が毎回注入します。あなたは workflow の `permissions:` ブロックで権限を絞るだけで、サンプル caller には既に記載済みです。

> **初回 PR に必須の secret は 1 つだけ:** `ANTHROPIC_API_KEY` か `CLAUDE_CODE_OAUTH_TOKEN` のいずれか一方です。2 つの PAT は本番向けで、最初は省略できます。「誰が用意するか」は [Secrets サマリ](#secrets-サマリ) を参照。
...
| `CODEX_REVIEW_REQUEST_TOKEN` | あなた（fine-grained PAT） | 任意（推奨） | `@codex review` を連携ユーザーとして投稿 |
| `LOOPPILOT_PUSH_TOKEN` | あなた（PAT / GitHub App） | 任意（本番実質必須） | repair commit push 用。required checks 再実行に必要 |
